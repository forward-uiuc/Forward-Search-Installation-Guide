...
This contains step-by-step guide to set up Forward Search for a particular domain. All steps are tested with Java 8. We are not sure if it works with earlier or newer versions of Java. Input is a URL template for the target domain and output is a web-based entity-semantic search engine for the domain. To understand the logic of each step below, please refer to Long Pham's master thesis at UIUC: https://github.com/forward-uiuc/Forward-Search-Installation-Guide/blob/master/PHAM-THESIS-2018.pdf
...
Go to the directory of the code repository you just cloned. Run `mvn package` to compile and create the excuatable jar file. 

Copy the csv created in the step above to folder `input` in the repository folder. The bash script below assumes the csv file is in that folder. Feel free to change the script. 
